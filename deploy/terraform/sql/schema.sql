...
-- Generations table: run after Users table exists (users.sql) if you use login.
IF NOT EXISTS (SELECT * FROM sys.tables WHERE name = 'Generations')
BEGIN
  CREATE TABLE dbo.Generations (
    Id                INT IDENTITY(1,1) PRIMARY KEY,
    UserId            INT NULL,  -- FK to Users.Id; NULL for legacy rows
    ...
    CONSTRAINT FK_Generations_Users FOREIGN KEY (UserId) REFERENCES dbo.Users(Id)
  );
