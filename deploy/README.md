...
**Order matters:** run **`users.sql`** first (creates the Users table for login; passwords stored as bcrypt hashes). Then run **`schema.sql`** (creates the Generations table and links it to Users for "My generations"). For forgot-password, run **`schema-password-reset.sql`** to add reset token columns to Users. Set **JWT_SECRET** in the Web App application settings for production. For password reset emails, set **PUBLIC_APP_URL** to your app’s base URL (e.g. `https://your-app.azurewebsites.net`) so reset links are correct; without it, the server logs the link in non-production.
